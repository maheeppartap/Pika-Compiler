...
class SemanticAnalysisVisitor extends ParseNodeVisitor.Default {
	...
	private void typeCheckError(ParseNode node, List<Type> operandTypes) {
		...
		// Check promotion
		if (node instanceof OperatorNode && promoter.isPromotable((OperatorNode) node)) return;
		if (node instanceof FunctionInvocationNode && promoter.isPromotable((FunctionInvocationNode) node)) return;
		if (node instanceof ArrayNode && promoter.isPromotable((ArrayNode) node)) return;
		if (node instanceof IndexNode && promoter.isPromotable((IndexNode) node)) return;
		if (node instanceof AssignmentNode && promoter.isPromotable(node)) return;
